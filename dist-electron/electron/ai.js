...
exports.AIService = {
    getModel() {
        return __awaiter(this, arguments, void 0, function* (taskType = 'review') {
            ...
            try {
                if (taskType === 'rewrite') {
                    const setting = settings_1.SettingsRepository.get('ai_model_rewrite');
                    if (setting && setting.value)
                        modelName = setting.value;
                }
                else if (taskType === 'review') {
                    const setting = settings_1.SettingsRepository.get('ai_model_review');
                    if (setting && setting.value)
                        modelName = setting.value;
                }
                else if (taskType === 'weekly') {
                    const setting = settings_1.SettingsRepository.get('ai_model_weekly');
                    if (setting && setting.value)
                        modelName = setting.value;
                }
            }
    ...
    analyzeTrade(trade) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            try {
                const model = yield this.getModel('review');
                let promptTemplate = DEFAULT_TRADE_REVIEW_PROMPT;
                const customPrompt = settings_1.SettingsRepository.get('ai_prompt_trade_review');
                ...
                const prompt = promptTemplate
                    ...
                    .replace('{{mistakes}}', ((_a = trade.mistakes) === null || _a === void 0 ? void 0 : _a.join(', ')) || 'None listed')
    ...
    coachJournal(entryContent, mood) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const model = yield this.getModel('review'); // Use review model for coaching
                const prompt = `
    ...
    queryTrades(query, trades) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b, _c, _d, _e, _f;
            try {
                const model = yield this.getModel('query');
                // --- PLANNER STEP ---
            ...
            Return ONLY valid JSON.
            `;
                const plannerResult = yield model.generateContent(PLANNER_PROMPT);
                ...
                const now = new Date();
                if ((_a = plan.filter) === null || _a === void 0 ? void 0 : _a.includes('last_7_days')) {
                    const cutoff = new Date(now);
                    cutoff.setDate(cutoff.getDate() - 7);
                    cutoff.setHours(0, 0, 0, 0);
                    filteredTrades = filteredTrades.filter(t => new Date(t.entryDateTime) >= cutoff);
                }
                else if ((_b = plan.filter) === null || _b === void 0 ? void 0 : _b.includes('last_30_days')) {
                    const cutoff = new Date(now);
                ...
                if ((_c = plan.filter) === null || _c === void 0 ? void 0 : _c.includes('winners')) {
                    filteredTrades = filteredTrades.filter(t => t.pnl && t.pnl > 0);
                }
                else if ((_d = plan.filter) === null || _d === void 0 ? void 0 : _d.includes('losers')) {
                    filteredTrades = filteredTrades.filter(t => t.pnl && t.pnl <= 0);
                }
                if ((_e = plan.filter) === null || _e === void 0 ? void 0 : _e.includes('long')) {
                    filteredTrades = filteredTrades.filter(t => t.direction === 'Long');
                }
                else if ((_f = plan.filter) === null || _f === void 0 ? void 0 : _f.includes('short')) {
                ...
                const fieldMap = (t) => {
                    ...
                    if (plan.required_fields && plan.required_fields.length > 0) {
                        plan.required_fields.forEach((f) => {
                            // Skip if already added
                            if (['id', 'market', 'date'].includes(f))
                                return;
                            // @ts-ignore
                            if (t[f] !== undefined)
                                projected[f] = t[f];
    ...
    weeklyReview(trades, _weekId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const model = yield this.getModel('weekly');
                ...
                const customPrompt = settings_1.SettingsRepository.get('ai_prompt_weekly_review');
                if (customPrompt && typeof customPrompt.value === 'string' && customPrompt.value.trim().length > 10) {
                    promptTemplate = customPrompt.value;
                }
                // 3. Construct Prompt
    ...
    rewriteJournal(text, context) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const model = yield this.getModel('rewrite');
...
{{notes_raw}}`;
                ...
                const customPrompt = settings_1.SettingsRepository.get('ai_prompt_rewrite');
                if (customPrompt && typeof customPrompt.value === 'string' && customPrompt.value.trim().length > 10) {
                    promptTemplate = customPrompt.value;
                }
                let prompt = '';
